...
export async function GET() {
  try {
    ...
    const startTs = new Date(currentYear, 0, 1).getTime();
    const endTs = new Date(currentYear, 11, 31, 23, 59, 59).getTime();
...
function processWorkload(tasks, members, timeEntries, startTs, endTs) {
  ...
  const cleanedTasks = tasks.map(task => {
    const hoursEstimate = task.time_estimate
      ? round1dec(task.time_estimate / 3600000)
      : 0;
    ...
    return {
      ...
      list: task.list,
      time_estimate_hours: hoursEstimate,
    };
  });
  ...
  const cleanedTimeEntries = timeEntries.map(entry => ({
    ...
    start: entry.start,
    duration_hours: round1dec(parseInt(entry.duration) / 3600000)
  }));
