...
import { RefreshCw, LogOut, Clock, Users, Calendar, TrendingUp, Coffee, Globe } from 'lucide-react';

const ACCESS_TOKEN_KEY = 'clickup_access_token';
// Pacific Time (PST/PDT) - Yeh time zone fix rahega converted time ke liye
const PACIFIC_TIMEZONE = 'America/Los_Angeles';

// Helper to get user's local timezone
const getUserLocalTimezone = () => {
    // Ye function user ke browser ka default timezone le leta hai, maslan 'Asia/Karachi'
    return Intl.DateTimeFormat().resolvedOptions().timeZone;
};

// Helper to format time in user's local timezone (UPAR WALA TIME)
const getLocalTime = (dateString, timeString) => {
    ...
    try {
        ...
        return dateTime.toLocaleTimeString('en-US', {
            ...
            minute: '2-digit',
            hour12: true,
            timeZone: userTimezone, // User ka local time zone
            timeZoneName: 'short'
        });
    } catch (err) {
        return null;
...
// Helper to convert specific date/time string to Pacific Time (NEECHY WALA TIME)
const getPacificTime = (dateString, timeString) => {
    ...
    try {
        ...
        return dateTime.toLocaleTimeString('en-US', {
            ...
            minute: '2-digit',
            hour12: true,
            timeZone: PACIFIC_TIMEZONE, // Hamesha Pacific Timezone
            timeZoneName: 'short'
        });
    } catch (err) {
        return null;
...
export default function TeamAttendancePage() {
    ...
    const filteredAttendance = attendance.filter(record => {
        if (selectedUser !== 'all') {
            // Check if selectedUser matches userId OR user name
    ...
    // Ab getPacificTime hamesha PST/PDT dega
    const getConvertedTime = getPacificTime;
    const CONVERTED_TIMEZONE_NAME = 'PST/PDT';
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 md:p-8 font-sans">
            ...
            {/* Date Range Filter Buttons & User Filter */}
            <div className="bg-white p-4 rounded-xl shadow-md mb-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm flex items-center">
                        <Globe className="w-4 h-4 mr-1"/> Your Local Time: {userTimezone}
                    </span>
                    <span className="px-3 py-1 bg-amber-100 text-amber-800 rounded-full text-sm">
                        Fixed Converted Time: {CONVERTED_TIMEZONE_NAME}
                    </span>
                </div>
            </div>
            ...
            {/* Loading Indicator */}
            {loading && (
                <div className="bg-indigo-5 border border-indigo-200 text-indigo-700 px-4 py-3 rounded-lg mb-6 flex items-center">
                                                            ...
                                                            <div>
                                                                <h4 className="font-semibold text-gray-700 mb-2 flex items-center">
                                                                    ...
                                                                    <span className="ml-2 text-xs font-normal text-gray-500">
                                                                        (Local: {userTimezone} | Converted: {CONVERTED_TIMEZONE_NAME})
                                                                    </span>
                                                                </h4>
                                                                <div className="space-y-2">
                                                                    {record.timeline && record.timeline.map((session, idx) => (
                                                                        <div key={`${record.uniqueId}_session_${idx}`} className="flex flex-col md:flex-row md:items-center text-sm bg-white p-3 rounded-lg shadow-sm">
                                                                            ...
                                                                            {/* NEECHHE WALA TIME: Converted (Pacific) time */}
                                                                            <div className="ml-3 text-xs text-blue-600">
                                                                                <div>
                                                                                    {`${CONVERTED_TIMEZONE_NAME}: ${getConvertedTime(record.date, session.checkIn)} → ${getConvertedTime(record.date, session.checkOut)}`}
                                                                                </div>
                                                                            </div>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="space-y-2">
                                                                        {record.breaks.map((brk, idx) => (
                                                                            <div key={`${record.uniqueId}_break_${idx}`} className="flex flex-col md:flex-row md:items-center text-sm bg-white p-3 rounded-lg shadow-sm">
                                                                                ...
                                                                                <span className="text-gray-600">
                                                                                    ...
                                                                                    <div className="text-xs text-blue-600 mt-1">
                                                                                        {`${CONVERTED_TIMEZONE_NAME}: ${getConvertedTime(record.date, brk.startTime)} → ${getConvertedTime(record.date, brk.endTime)}`}
                                                                                    </div>
                                                                                </span>
